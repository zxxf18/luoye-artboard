...
try {
 ...
 for(const brush of ['crayon','pencil','spray']) {
  ...
  check($('painting').dataset.cursor==='pen:'+brush&&getComputedStyle($('painting')).cursor.includes('data:image/svg+xml'),'WKWebView illustrated cursor '+brush);
 ...
 $('music-open').click();$('ui-sounds').checked=false;$('ui-sounds').dispatchEvent(new Event('change',{bubbles:true}));$('music-close').click();const silent=oscillators;
 ...
 const image=new Image();image.src=getComputedStyle($('painting')).cursor.match(/url\("([^"]+)/)[1];await image.decode();check(image.width===40&&image.height===40,'WKWebView decodes actual cursor SVG');
