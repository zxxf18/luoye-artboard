...
await pause(3500);
const music=await window.JSHWMusicState();
...
const saved=await window.JSHWFlushBeforeClose();
...
} catch(error) { return {failure:error.message,stack:error.stack,music:await window.JSHWMusicState?.()}; }
