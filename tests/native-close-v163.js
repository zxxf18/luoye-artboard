...
const choose=async value=>{const pending=window.LUOYERequestClose();await pause(40);check(document.getElementById('close-dialog').open,'显示三选退出窗口');document.querySelector('#close-dialog button[value="'+value+'"]').click();return pending;};
...
try{
 check((await window.LUOYERequestClose()).action==='exit','空白未修改直接退出');
 ...
 if(fileChecks){
  ...
  const attempt=window.LUOYERequestClose();await pause(30);
