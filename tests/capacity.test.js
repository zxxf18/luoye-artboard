...
test('10000 instances per layer and 50000 per project have explicit validated boundaries',()=>{
  ...
  const layer={id:'a',name:'草地',width:1,height:1,x:0,y:0,scale:1,rotation:0,opacity:1,visible:true,image,sprites:Array.from({length:10000},()=>({group:0,x:0,y:0,size:16,opacity:1})),spriteGroups:[{frameDuration:160,frames:[{width:1,height:1,image}]}]};
  ...
  assert.throws(()=>validateProject({...project,layers:[...project.layers,{...layer,id:'extra',sprites:[layer.sprites[0]]}]}),/50,000/);
});
