...
if(!localStorage.getItem(marker)){
 ...
 document.getElementById('title').value='必须留下的旧画';await window.JSHWFlushBeforeClose();
...
const current=await window.JSHWFlushBeforeClose();assert(current.project.title!=='必须留下的旧画','Old work opened instead of a blank sheet');
...
const items=await new Promise((resolve,reject)=>{const request=indexedDB.open('jshw-studio',2);request.onsuccess=()=>{const db=request.result,r=db.transaction('gallery').objectStore('gallery').getAll();r.onsuccess=()=>{db.close();resolve(r.result);};r.onerror=()=>reject(r.error);};request.onerror=()=>reject(request.error);});
