...
export function mountMaterials({engine,run,toast}) {
  ...
  panel.innerHTML=`<label data-option="${drawing}">色彩选择 <select id="paint-source"><option value="color">颜色</option><option value="texture-0">纹理 1</option><option value="texture-1">纹理 2</option><option value="custom">输入图片</option></select></label><label data-option="pen">画纸材质 <select id="paper-grain"><option value="none">光滑</option><option value="grain-0">纸纹 1</option><option value="grain-1">纸纹 2</option></select></label><label data-option="pen">纸纹强度 <input id="paper-grain-strength" type="range" min="0" max="100" value="70"></label><input id="paint-texture-file" type="file" accept="image/png,image/jpeg,image/webp" hidden>`;
  // The playroom shelf owns these controls directly; keeping the temporary
  // wrapper detached prevents duplicate controls when the detail bar is
  // expanded into the shelf.
  for(const label of panel.querySelectorAll('label'))document.querySelector('.primary-brush-options').append(label);for(const item of document.querySelector('.primary-brush-options').querySelectorAll('[data-option]'))item.hidden=!item.dataset.option.split(' ').includes('pen');let source='color',grain='none';
