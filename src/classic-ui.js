...
export function mountClassic({setTool,getColor,setColor,clearAnimations,quickNew,resetSettings}){
  ...
  const header=document.querySelector('.header-actions');if(quickNew)el('new-quick').onclick=quickNew;if(resetSettings)el('reset-settings').onclick=resetSettings;header.prepend(el('undo'),el('redo'));for(const [id,icon,name] of [['undo','undo','撤销'],['redo','redo','重做'],['new-quick','plus','新建'],['reset-settings','undo','重置'],['open','folder','打开'],['save','save','保存'],['export','download','导出'],['gallery','folder','画夹']]){el(id).innerHTML=playfulIcon(icon)+`<span>${name}</span>`;el(id).className='header-command';}
