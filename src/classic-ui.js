...
export function mountClassic({setTool,getColor,setColor,clearAnimations}){
  ...
  function chooseBrush(value){if(el('painting').dataset.tool==='pen')sizes[currentBrush]=Number(el('size').value);openLibrary(false);currentBrush=value;el('brush').value=value;el('size').value=sizes[value]||BRUSHES.find(b=>b.id===value).size;el('size').dispatchEvent(new Event('input',{bubbles:true}));setTool('pen');sync();document.dispatchEvent(new Event('controlschange'));}
