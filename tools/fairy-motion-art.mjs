...
function face(x,y,scale=1){return group(`translate(${x} ${y}) scale(${scale})`,circle(-22,-4,7,'#594339')+circle(22,-4,7,'#594339')+circle(-24,-7,2,'white')+circle(20,-7,2,'white')+ellipse(-39,12,11,6,'#f3a09a')+ellipse(39,12,11,6,'#f3a09a')+line('M-10 15 Q0 27 10 15','#755345',4));}
...
function flower(kind,variant,t){
 const grow=kind===0?.48+t*.52:1,sway=Math.sin(t*pi*2+variant)*8;
 ...
 function blossom(x,y,s=1){
  ...
  return group(`translate(${x} ${y}) scale(${s})`,p);
 }
...
function grass(kind,variant,t){
 ...
 for(let i=0;i<count;i++){
  ...
  const tip=x+a*12+Math.sin(t*2*pi+variant+i*.16)*22,y=470-height;
 ...
 return b+leaf(250,470,70,.42)+leaf(265,465,-33,.39);
}
function bubbles(kind,variant,t){
 let b='';for(let i=0;i<4;i++){
  const r=[67,43,28,18][i]*(1+variant*.035),x=[238,351,151,359][i]+Math.sin(t*2*pi+i)*12,y=[310,190,123,399][i]-Math.sin(t*2*pi+i)*18;
...
function smoke(variant,t){let b='';for(let i=4;i>=0;i--){const r=29+i*(12+variant),x=256+Math.sin(t*2*pi+i*.8+variant)*36,y=425-i*70; b+=group(`translate(${x} ${y}) rotate(${Math.sin(t*2*pi)*8})`,circle(0,0,r,'url(#smoke)')+path(`M${-r*.5} 8C${-r*.5} ${-r*.6} ${r*.65} ${-r*.6} ${r*.3} 6C${r*.12} 28 ${-r*.25} 14 0 2`,'none','#c1cbd6',4));}return b;}
function dandelion(variant,t){
 ...
 }return b+circle(sx,sy,17,'#c5b887');
}
function ladybug(variant,t){
 ...
 b+=ellipse(256,157,69,56,'#6b6256')+ellipse(256,169,51,39,'#ffe0a1')+face(256,166,.7)+line('M221 119Q180 88 192 69M286 118Q327 89 316 68','#6b6256',6)+circle(193,70,10,'#f7bc6b')+circle(316,68,10,'#f7bc6b');
...
function butterfly(variant,t){
 ...
 b+=ellipse(256,284,22,81,'#d99e62','#a4714e',4)+circle(256,202,34,'#ffe1a0','#b9895b',4)+face(256,203,.48)+line('M239 177Q199 125 213 120M270 176Q304 128 297 119','#987148',5)+circle(213,120,7,'#e9a8bc')+circle(298,119,7,'#e9a8bc');return b;
}
...
function fallingLeaf(kind,variant,t){
 ...
 return group(`translate(${Math.sin(t*2*pi)*18} ${Math.cos(t*2*pi)*13}) rotate(${Math.sin(t*2*pi)*18+variant*8} 256 256)`,b);
}
